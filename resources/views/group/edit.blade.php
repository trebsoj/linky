...
<div class="max-w-2xl">
    <form action="{{route('group.update', $group->id)}}" method="POST" class="bg-white rounded-lg shadow-md p-6 border border-gray-200 space-y-6" novalidate>
        @method('PUT')
        @csrf

        <!-- Group Name -->
        <div>
            <label for="vGroupName" class="block text-sm font-semibold text-gray-700 mb-2">Group Name</label>
