...
<div class="max-w-2xl">
    <form action="{{route('variable.update', $variable->id)}}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700 space-y-6" novalidate>
        @method('PUT')
        @csrf

        <!-- Variable Key -->
        <div>
            <label for="key" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Variable Key</label>
        ...
        <!-- Variable Value -->
        <div>
            <label for="value" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Variable Value</label>
            <input type="text" name="value" placeholder="The value to replace with" 
                class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
